...
public class ReverseMuttyGenerator : IIncrementalGenerator
{
    ...
    {withoutAttr.ToFullString().Trim()}");
        ...
        if (properties.Any())
        {
            ...
            foreach (var prop in properties)
            {
                var conversion = ToImmutableExpression($"source.{prop.Name}", prop.OriginalType);
                sb.AppendLine($"            {prop.Name} = {conversion},");
            }
            sb.Append("        };");
        }
        ...
        // 定义递归转换函数（用于生成表达式）
        string ToImmutableExpression(string sourceExpr, ITypeSymbol originalType)
        {
            if (originalType is INamedTypeSymbol named)
            {
                if (listType != null && SymbolEqualityComparer.Default.Equals(named.OriginalDefinition, listType))
                {
                    ...
                    var elementConversion = ToImmutableExpression("item", elementType);
                    return $"global::System.Collections.Immutable.ImmutableList.ToImmutableList({sourceExpr}.Select(item => {elementConversion}))";
                }
                if (dictType != null && SymbolEqualityComparer.Default.Equals(named.OriginalDefinition, dictType))
                {
                    ...
                    var valueConversion = ToImmutableExpression("kvp.Value", valueType);
                    return $"global::System.Collections.Immutable.ImmutableDictionary.ToImmutableDictionary({sourceExpr}, kvp => kvp.Key, kvp => {valueConversion})";
                }
            }
            return sourceExpr;
        }

        string ToMutableExpression(string sourceExpr, ITypeSymbol originalType)
        {
            if (originalType is INamedTypeSymbol named)
            {
                if (listType != null && SymbolEqualityComparer.Default.Equals(named.OriginalDefinition, listType))
                {
                    var elementType = named.TypeArguments[0];
                    var elementConversion = ToMutableExpression("item", elementType);
                    return $"global::System.Linq.Enumerable.ToList({sourceExpr}.Select(item => {elementConversion}))";
                }
                if (dictType != null && SymbolEqualityComparer.Default.Equals(named.OriginalDefinition, dictType))
                {
                    var keyType = named.TypeArguments[0];
                    var valueType = named.TypeArguments[1];
                    var valueConversion = ToMutableExpression("kvp.Value", valueType);
                    return $"global::System.Linq.Enumerable.ToDictionary({sourceExpr}, kvp => kvp.Key, kvp => {valueConversion})";
                }
            }
            return sourceExpr;
        }
